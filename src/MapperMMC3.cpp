...
MapperMMC3::MapperMMC3(Cartridge &cart, std::function<void()> interrupt_cb,
    ...
    : Mapper(cart, Mapper::MMC3), m_targetRegister(0), m_prgBankMode(false),
      m_chrInversion(false), m_bankRegister{}, m_irqEnabled(false),
      m_irqCounter(0), m_irqLatch(0), m_irqReloadPending(false),
      m_prgRam(32 * 1024), m_mirroringRam(4 * 1024), m_mirroring(Horizontal),
      m_mirroringCallback(mirroring_cb), m_interruptCallback(interrupt_cb) {
  ...
  if (addr >= 0x6000 && addr <= 0x7FFF) {
    ...
    if (!(addr & 0x01)) {
      ...
      if (m_prgBankMode == 0) {
        ...
        m_prgBank2 =
            &m_cartridge.getROM()[m_cartridge.getROM().size() - 0x4000];
        m_prgBank3 =
            &m_cartridge.getROM()[m_cartridge.getROM().size() - 0x2000];
