...
void PPU::reset() {
  m_longSprites = m_generateInterrupt = m_greyscaleMode = m_vblank =
      m_spriteOverflow = false;
  m_showBackground = m_showSprites = m_evenFrame = m_firstWrite = true;
  m_bgPage = m_sprPage = Low;
  m_dataAddress = m_cycle = m_scanline = m_spriteDataAddress = m_fineXScroll =
      m_tempAddress = 0;
...
void PPU::step() {
  ...
  case PreRender:
    ...
    if (m_cycle >= ScanlineEndCycle -
                       (!m_evenFrame && m_showBackground && m_showSprites)) {
    ...
    if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots) {
      ...
      if (m_showBackground) {
        ...
        if (!m_hideEdgeBackground || x >= 8) {
          ...
          addr = (tile * 16) +
                 ((m_dataAddress >> 12 /*y % 8*/) & 0x7); // Add fine y
          addr |= m_bgPage
                  << 12; // set whether the pattern is in the high or low page
          ...
          bgOpaque = bgColor; // flag used to calculate final pixel with the
                              // sprite pixel

          // fetch attribute and calculate higher two bits of palette
          addr = 0x23C0 | (m_dataAddress & 0x0C00) |
                 ((m_dataAddress >> 4) & 0x38) | ((m_dataAddress >> 2) & 0x07);
      ...
      if (m_showSprites && (!m_hideEdgeSprites || x >= 8)) {
        for (auto i : m_scanlineSprites) {
          ...
          if (!m_sprZeroHit && m_showBackground && i == 0 && sprOpaque &&
              bgOpaque) {
            m_sprZeroHit = true;
          }
...
Byte PPU::readOAM(Byte addr) { return m_spriteMemory[addr]; }

void PPU::writeOAM(Byte addr, Byte value) { m_spriteMemory[addr] = value; }

void PPU::doDMA(const Byte *page_ptr) {
  std::memcpy(m_spriteMemory.data() + m_spriteDataAddress, page_ptr,
              256 - m_spriteDataAddress);
  if (m_spriteDataAddress)
    std::memcpy(m_spriteMemory.data(), page_ptr + (256 - m_spriteDataAddress),
                m_spriteDataAddress);
}
...
Byte PPU::getOAMData() { return readOAM(m_spriteDataAddress); }
...
void PPU::setOAMAddress(Byte addr) { m_spriteDataAddress = addr; }

void PPU::setOAMData(Byte value) { writeOAM(m_spriteDataAddress++, value); }
...
Byte PPU::read(Address addr) { return m_bus.read(addr); }
