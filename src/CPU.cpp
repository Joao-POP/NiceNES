...
Byte CPU::pullStack() { return m_bus.read(0x100 | ++r_SP); }
...
bool CPU::executeImplied(Byte opcode) {
  ...
  case JMPI: {
    ...
    r_PC = m_bus.read(location) | m_bus.read(Page | ((location + 1) & 0xff))
                                      << 8;
  } break;
  case PHP: {
...
bool CPU::executeType1(Byte opcode) {
  if ((opcode & InstructionModeMask) == 0x1) {
    ...
    auto op =
        static_cast<Operation1>((opcode & OperationMask) >> OperationShift);
    ...
    case IndexedIndirectX: {
      ...
      location = m_bus.read(zero_addr & 0xff) |
                 m_bus.read((zero_addr + 1) & 0xff) << 8;
    } break;
    case ZeroPage:
...
bool CPU::executeType2(Byte opcode) {
  if ((opcode & InstructionModeMask) == 2) {
    Address location = 0;
    auto op =
        static_cast<Operation2>((opcode & OperationMask) >> OperationShift);
    auto addr_mode =
